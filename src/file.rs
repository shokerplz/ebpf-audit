...
pub struct TraceOpenProgram {
    ringbuf: libbpf_rs::RingBuffer<'static>,
    _skel: &'static mut TraceOpenSkel<'static>,
    db_conn: Connection,
    rx: tokio::sync::mpsc::Receiver<RustFileEvent>,
}

async fn write_batch(db_conn: &Connection, batch: &mut Vec<RustFileEvent>) {
    if batch.is_empty() {
        return;
    }

    debug!("Writing batch of {} file events to DB", batch.len());
...
async fn check_batch(db_conn: &Connection, batch: &mut Vec<RustFileEvent>) {
    if batch.is_empty() {
        return;
    }

    debug!("Checking batch of {} file events against DB", batch.len());
    let db_batch = mem::take(batch);
    let new_events = db_conn
        .call(move |c| -> Result<HashSet<(String, String, String)>> {
            let tx = c.transaction()?;
            tx.execute(
                "CREATE TEMP TABLE IF NOT EXISTS batch_check (comm TEXT, exe TEXT, path TEXT)",
                [],
            )?;
            {
                let mut insert_stmt =
                    tx.prepare("INSERT INTO batch_check (comm, exe, path) VALUES (?1, ?2, ?3)")?;
                for event in &db_batch {
                    insert_stmt.execute(params![&event.comm, &event.exe, &event.path])?;
                }
            }
            let mut stmt = tx.prepare(
                "SELECT DISTINCT b.comm, b.exe, b.path
                 FROM batch_check b
                 WHERE NOT EXISTS (
                   SELECT 1 FROM files_opened f
                   WHERE f.comm = b.comm AND f.exe = b.exe AND f.path = b.path
                 )",
            )?;
...
impl TraceOpenProgram {
    pub fn new(db_conn: Connection) -> Result<Self> {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<RustFileEvent>(MAX_EVENTS);
        ...
        loop {
            ...
            while let Ok(event) = self.rx.try_recv() {
                ...
                if batch.len() >= BATCH_SIZE {
                    match mode {
                        Mode::CollectData => {
                            time_it!("file::write_batch", write_batch(&self.db_conn, &mut batch))
                                .await;
                        }
                        Mode::Analysys => {
                            time_it!("file::check_batch", check_batch(&self.db_conn, &mut batch))
                                .await;
                        }
                    }
                }
